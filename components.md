...
The bar width is set inline as `style="width: X%"`. The default gradient is silver (for the winner card). Override inline for competitors:
- Raw Claude Code: `background: linear-gradient(90deg, var(--cyan), #0e7490)`
- Middle option: `background: linear-gradient(90deg, var(--accent3), #9d174d)`
- Winner: default gradient (green-to-silver)

---

## JavaScript Patterns

All JS is vanilla — no libraries except Lucide icons. Scripts live in a single `<script>` block at the bottom of `<body>`.

### Theme Toggle

See `README.md` — full implementation there.
...
Close on any nav link click (prevents menu staying open after navigation):
```js
...
The hamburger button is `display:none` above `768px` and shown below it. The `.nav-links` list is `display:none` by default and becomes a vertical dropdown when `.active` is added.
...
The `-40px` bottom rootMargin means the element must be 40px into the viewport before triggering — prevents elements activating just as they appear at the very edge.
...
function switchWorkflow(mode) {
  ...
  // mode: 'dev' | 'devops' | 'silver' (or equivalent)
  const tabIndex = mode === 'dev' ? 1 : mode === 'devops' ? 2 : 0;
  document.querySelectorAll('.workflow-tab')[tabIndex].classList.add('active');
  document.getElementById('workflow-' + mode).classList.add('active');
}
```

Each tab button calls `switchWorkflow('dev')` etc. via `onclick`. The corresponding `.workflow-panel` div has `id="workflow-dev"` etc. Inactive panels use `display:none`.
...
Must be called once after DOM is ready — place at the end of the `<script>` block, after all other JS. Icons render as `<svg>` replacing `<i data-lucide="name">` elements.
...
| Value | Usage |
|-------|-------|
| `1` | Hero content container (above glow orbs) |
| `100` | Fixed nav |
| `200` | Nav search results dropdown |

No other explicit z-index values. The layering relies on DOM order for everything else.
