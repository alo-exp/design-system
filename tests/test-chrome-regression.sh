#!/usr/bin/env bash
# Regression guards for Ālo site kit chrome + card hover parity.
set -euo pipefail
...
WORKFLOWS_FIXTURE="$REPO_ROOT/examples/fixtures/help-workflows-h1.html"
CHROME_CSS="$KIT/chrome.css"
TOKENS_CSS="$KIT/tokens.css"
NEUTRAL_CSS="$KIT/neutral-variants.css"
HOMEPAGE="$REPO_ROOT/examples/fixtures/homepage-card-hover.html"

# 1. Help workflows h1 must not carry a spurious APO badge.
assert_contains "workflows h1 is Orchestration Workflows" '<h1>Orchestration Workflows</h1>' "$WORKFLOWS_FIXTURE"
assert_not_contains "workflows h1 has no inline APO badge span" 'Orchestration Workflows[[:space:]]*<span[^>]*>APO</span>' "$WORKFLOWS_FIXTURE"
...
assert_contains "neutral-variants s3 applies unified card hover shadow" 'html\[data-neutral-variant="s3"\] \.pain-card:hover' "$NEUTRAL_CSS"
assert_contains "neutral-variants s3 hover uses card-shadow-hover token" 'box-shadow:var\(--card-shadow-hover\) !important' "$NEUTRAL_CSS"

# 4. Single Alpha Honesty callout in #proof.
